...
const getSurvivorCount = (data) => {
	const survivedCount = data.filter(p => p.fields.survived === 'Yes')
	return survivedCount.length
}
...
const getCasualityCount = (data) => {
	const deceasedCount = data.filter(p => p.fields.survived === 'No')
	return deceasedCount.length
}
...
const countPassengersInClass = (data, pclass) => {
	const classCount = data.filter(p => p.fields.pclass === pclass)
...
const getSurvivorCountForClass = (data, pclass) => {
	const surivorClassCount = data.filter(p => p.fields.pclass === pclass).filter(p => p.fields.survived === 'Yes')
	return surivorClassCount.length
}
...
const getCasualityCountForClass = (data, pclass) => {
	const deceasedClassCount = data.filter(p => p.fields.pclass === pclass).filter(p => p.fields.survived === 'No')
	return deceasedClassCount.length
}
...
const getMinAge = (data) => {
	...
	const ageArr = validAges.map(p => p.fields.age)
...
const getEmbarkedCount = (data, embarked) => {
	const embarkedCount = data.filter(p => p.fields.embarked === embarked)
...
const getMinFare = (data) => {
	const validFares = data.filter(p => p.fields.fare !== undefined)
	const fareArr = validFares.map(p => p.fields.fare)
	return Math.min(...fareArr)
}
...
const getPassengersByGender = (data, gender) => {
	const genderCount = data.filter(p => p.fields.sex === gender)
	return genderCount.length
}
...
const getSurvivorsByGender = (data, gender) => {
	const survivorGenderCount = data.filter(p => p.fields.survived === 'Yes').filter(p => p.fields.sex === gender)
	return survivorGenderCount.length
}
...
const getCasualitiesByGender = (data, gender) => {
	const deceasedGenderCount = data.filter(p => p.fields.survived === 'No').filter(p => p.fields.sex === gender)
	return deceasedGenderCount.length
}
...
const getTotalFare = (data) => {
	const fareTotal = data.filter(p => p.fields.fare !== undefined).reduce((acc, p) => acc + p.fields.fare, 0)
	return fareTotal
}
...
const getAverageFare = (data) => {
	const fareTotal = data.filter(p => p.fields.fare !== undefined).reduce((acc, p) => acc + p.fields.fare, 0)
	const passenegersWhoPaid = data.filter(p => p.fields.fare !== undefined)
	return fareTotal / passenegersWhoPaid.length
}
...
const getAverageAge = (data) => {
	const ageTotal = data.filter(p => p.fields.age !== undefined).reduce((acc, p) => acc + p.fields.age, 0)
	const passengerCount = data.filter(p => p.fields.age !== undefined)
	return ageTotal / passengerCount.length
}
...
const getAverageAgeByGender = (data, gender) => {
	const passengerCount = data.filter(p => p.fields.sex === gender).filter(p => p.fields.age !== undefined)
	const ageGenderTotal = passengerCount.reduce((acc, p) => acc + p.fields.age, 0)
	return ageGenderTotal / passengerCount.length
}
